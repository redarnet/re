...
void	put_error(char *message, struct data_s data)
{
	int i;
...
int		bmp_data(int fd, struct data_s data, int padsize)
{
...
int		take_screenshot(struct data_s data)
{
	int padsize;
	int fd;

	//run_game(data);
	padsize = (4 - ((int)data.x * 3) % 4) % 4;
	if ((fd = open("screenshot.bmp", O_WRONLY |
	O_CREAT, 0777 | O_TRUNC | O_APPEND)) < 0)
		return (0);
	bmp_header(fd, data.y, data.x, padsize);
	bmp_data(fd, data, padsize);
	close(fd);
	return (1);
}

void	make_screenshot(struct data_s data, char *str)
{
	if (ft_strncmp("--save", str, 7) != 0)
		put_error("ERROR\nUnrecognized second argument\n", data);
	data.sreenshot = 1;
	write(1, "Saving screenshot...\n", 21);
	if (!take_screenshot(data))
